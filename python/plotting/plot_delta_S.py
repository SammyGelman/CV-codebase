...
def get_sizes(s):
    ...
    m = re.search(r'(?<=final_models_gpu_l)\w+', s)
    if not m:
        return 0
...
for s in size:
    delta_S.append(np.mean(np.abs(np.genfromtxt('final_models_gpu_l'+str(s)+'/delta_S.dat')[:,1])))
    # delta_S.append(np.mean(np.abs(np.genfromtxt('final_models_l'+str(s)+'/delta_S.dat')[:,1])))
    error.append(np.mean(np.abs(np.genfromtxt('final_models_gpu_l'+str(s)+'/error.dat')[:,1])))
